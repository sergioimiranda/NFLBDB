...
dfplays = KickOffData %>% 
  dplyr::filter(gameId == game) %>% 
  dplyr::filter(playId == play)  
...
df_track <- df_track %>%
  dplyr::select(gameId, playId, x, y, s, dir, event, displayName, jerseyNumber, frameId, team)

df_track$team <- case_when(df_track$displayName == "football" ~ NA_character_, TRUE ~ df_track$team)
...
df_track <- df_track %>%
  ...
  dplyr::select(gameId, playId, frameId, event, team = team_name, jerseyNumber, displayName, x, y, s, v_theta, v_x, v_y)
...
play_frames <- plot_field() + 
  ...
  # ball
  geom_point(
    data = df_track %>% dplyr::filter(displayName == "football"),
    mapping = aes(x = x, y = y),
    fill = "#935e38", colour = "#d9d9d9",
    shape = 21, alpha = 1, size = 4, stroke = 1
  ) +
  # title 
  labs(title = dfplays$playDescription) +
  # animation stuff
